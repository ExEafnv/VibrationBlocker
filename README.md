...
**Q: 来电没振动了怎么办？**
**A: 把「电话」或 `com.android.server.telecom` 加入白名单。可先用 LSPosed 日志查看来电时 `opPkg=` 的值，再精准添加。**

**Q: 修改配置后不生效？**
**A: 配置实时生效，无需重启。若仍无效，检查 LSPosed 日志中是否出现 `VibBlocker` 关键字。**

**Q: 首次启用后一定要重启吗？**
**A: 是。Hook 系统框架只能在开机时注入，首次激活必须重启手机。**

**Q: 显示「未激活」但功能正常？**
**A: 尝试重新安装 APK 并重启手机。若问题持续，请在 Issues 中附上 LSPosed 日志。**
